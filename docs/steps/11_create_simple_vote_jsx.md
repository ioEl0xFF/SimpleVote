# Step 11: simple-vote-ui/src/SimpleVote.jsx の作成

このステップでは、`SimpleVote` のロジックを `PollRegistry` コントラクトと連携させるための新しい React コンポーネント `SimpleVote.jsx` を作成します。このコンポーネントは、`DynamicVote.jsx` と `WeightedVote.jsx` と同様に、`pollId` を受け取り、`PollRegistry` の `vote` および `cancelVote` 関数を呼び出します。

## 11.1. ファイルの作成

`simple-vote-ui/src/SimpleVote.jsx` という名前で新しいファイルを作成します。

## 11.2. コンポーネントのコード

以下のコードを `simple-vote-ui/src/SimpleVote.jsx` に記述します。

```javascript
// simple-vote-ui/src/SimpleVote.jsx
...
function SimpleVote({ signer, pollId, showToast, onBack }) {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            const [id, pollType, owner, topic, startTime, endTime, choiceNames, voteCounts] = await registry.getPoll(pollId);
            setTopic(topic);
